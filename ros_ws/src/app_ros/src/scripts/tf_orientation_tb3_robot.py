...
joint_state_angle_x = 0.0

#callback function the joint_states (position) of the robot 
def jointStateCallback(joint_state_msg):
    
    global joint_state_angle_x
    rospy.loginfo(rospy.get_caller_id() + " X joint_base_mid angle is = %s rad", '{:.2f}'.format(joint_state_msg.position[0]))
    joint_state_angle_x = joint_state_msg.position[0]
...
def odomPoseCallback(odom_msg):
    ...
    global joint_state_angle_x 
    translation_x = 0.14
    translation_joint_base_mid = 0.25   
    ...
    #print the position in degrees base mid joint robot arm
    rospy.loginfo(rospy.get_caller_id() + " X joint_base_mid position in world = %s m", '{:.2f}'.format(odom_msg.pose.pose.position.x - translation_x + math.sin(joint_state_angle_x)*translation_joint_base_mid))
...
if __name__ == '__main__':
    try:
       ...
       #subscribe to the joint_states topic
        joint_state_topic = "/rrbot/joint_states"
        joint_state_subscriber = rospy.Subscriber(joint_state_topic, JointState, jointStateCallback) 
