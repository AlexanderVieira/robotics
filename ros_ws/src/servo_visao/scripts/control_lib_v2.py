...
def cartesian_control(robot_pose, goal_stamped, K_v, K_omega, max_lin=0.5, max_ang=0.5, threshold=0.30):
    """
...
def polar_control(robot_pose,goal_stamped,K_rho,K_alpha, K_beta, max_lin=0.5, max_ang=0.5, threshold=0):
    """
    This function computes the control signal to guides the 
    robot to the desired goal. It's based on the Cartesian
    Control Algorithm
    ...
    rospy.loginfo(rospy.get_caller_id() + " Posição goal_stamped X: %s m", '{:.2f}'.format(goal_stamped.point.x))
    rospy.loginfo(rospy.get_caller_id() + " Posição goal_stamped Y: %s m", '{:.2f}'.format(goal_stamped.point.y))
    rospy.loginfo(rospy.get_caller_id() + " K_rho: %s", '{:.2f}'.format(K_rho))
    rospy.loginfo(rospy.get_caller_id() + " K_alpha: %s", '{:.2f}'.format(K_alpha))
    rospy.loginfo(rospy.get_caller_id() + " K_beta: %s", '{:.2f}'.format(K_beta))
    rospy.loginfo(rospy.get_caller_id() + " Regulação parcial: %s m", '{:.2f}'.format(threshold))
    ...
    x_d = goal_stamped.point.x
    y_d = goal_stamped.point.y
    theta_d = goal_stamped.point.z
    ...
    rospy.loginfo(rospy.get_caller_id() + " rho: %s", '{:.2f}'.format(rho))
    rospy.loginfo(rospy.get_caller_id() + " alpha: %s", '{:.2f}'.format(alpha))
    rospy.loginfo(rospy.get_caller_id() + " beta: %s", '{:.2f}'.format(beta))    
...
def tracking_control(donkey_pose, master_pose, gains_cart, max_vel, threshold, error_int):
    """
    This function computes the control signal to guides the 
    robot to the desired goal. It's based on the Cartesian
    Control Algorithm
    ...
    rospy.loginfo(rospy.get_caller_id() + " Posição donkey_pose X: %s m", '{:.2f}'.format(donkey_pose.x))
    rospy.loginfo(rospy.get_caller_id() + " Posição donkey_pose Y: %s m", '{:.2f}'.format(donkey_pose.y))
    rospy.loginfo(rospy.get_caller_id() + " Posição master_pose X: %s m", '{:.2f}'.format(master_pose.point.x))
    rospy.loginfo(rospy.get_caller_id() + " Posição master_pose Y: %s m", '{:.2f}'.format(master_pose.point.y))
    rospy.loginfo(rospy.get_caller_id() + " Ganho K_v: %s", '{:.2f}'.format(gains_cart[0]))
    rospy.loginfo(rospy.get_caller_id() + " Ganho K_omega: %s", '{:.2f}'.format(gains_cart[1]))
    rospy.loginfo(rospy.get_caller_id() + " Ganho K_int: %s", '{:.2f}'.format(gains_cart[5]))
    rospy.loginfo(rospy.get_caller_id() + " Regulação parcial: %s m", '{:.2f}'.format(threshold))
    
    # Recovering control gains
    K_v = gains_cart[0]    
    K_omega = gains_cart[1]
    K_int = gains_cart[5]
    ...
    # Computing the position error
    error_x = master_pose.point.x - donkey_pose.x
    error_y = master_pose.point.y - donkey_pose.y
    error_lin = round(math.sqrt(error_x**2 + error_y**2)-threshold,3)
...
def pbvs(image_point, camera_matrix, robot_pose, gains_cart, vel_lim, threshold):
    ...
    cmd_vel = cartesian_control(robot_pose, goal_stamped, k_v, k_w, max_lin, max_ang, threshold)
...
# Version: 1.0                              
# Date: 29-03-2021    
def pbvs_v1(image_point, camera_matrix, robot_pose, gains_cart, vel_lim, threshold):
        
    # Recovering gains
    K_rho = gains_cart[2]
    K_alpha = gains_cart[3]
    K_beta = gains_cart[4]
    ...
    goal_stamped = get_goal_stamped(cam_point)
    #rospy.loginfo(goal_stamped)
    cmd_vel = polar_control(robot_pose, goal_stamped, K_rho, K_alpha, K_beta, max_lin, max_ang, threshold)
    #rospy.loginfo(cmd_vel)
...
# Version: 1.0                              
# Date: 29-03-2021    
def pbvs_v2(image_point, camera_matrix, robot_pose, gains_cart, vel_lim, threshold, error_int):
    ...
    goal_stamped = get_goal_stamped(cam_point)
    #rospy.loginfo(goal_stamped)
    cmd_vel, errors = tracking_control(robot_pose,goal_stamped, gains_cart, vel_lim, threshold, error_int)
    #rospy.loginfo(cmd_vel)
     #rospy.loginfo(errors)

    return cmd_vel, errors
