...
def camera_main():
    ...
    pub_goal_base = rospy.Publisher('goal_base', PolygonStamped, queue_size=10)  # send control signals
    ...
    while not rospy.is_shutdown():
        ...
        if base is not None:
            if len(base) > 0:
                if len(base) == 1:
                    goal_base = PolygonStamped()
                    goal_base.header.frame_id = 'BASE_POINTS'
                    goal_base.polygon.points = [Point32(x=base[0][0], y=base[0][1], z=1)] 
                    pub_goal_base.publish(goal_base)           
                else:
                    goal_base = PolygonStamped()
                    goal_base.header.frame_id = 'BASE_POINTS'
                    goal_base.polygon.points = [Point32(x=base[0][0], y=base[0][1], z=1), Point32(x=base[1][0], y=base[1][1], z=1)]
                    pub_goal_base.publish(goal_base)
                    # if len(goal_base.polygon.points) > 0:
                    #     pub_goal_base.publish(goal_base)
        else:

            goal_base = PolygonStamped()
            goal_base.header.frame_id = 'BASE_POINTS'
            goal_base.polygon.points = [Point32(x=0.0, y=0.0, z=0.0)]
            pub_goal_base.publish(goal_base)
